...
const WellnessPackageCard: React.FC<WellnessPackageCardProps> = ({
  package: pkg,
}) => {
    ...
    <div className="max-w-md w-full bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300">
      ...
      <div className={`${packageColor.bg} p-6 text-white`}>
        <div className="flex justify-between items-start mb-2">
          ...
          <div
            className={`px-3 py-1 rounded-full text-sm font-semibold ${pkg.paymentStatus === "Paid" ? "bg-green-400" : "bg-amber-400"}`}
          >
            {pkg.paymentStatus}
          </div>
        </div>
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <FaCheckCircle
              className={`w-4 h-4 ${pkg.paymentStatus === "Paid" ? "text-green-500" : "text-amber-500"}`}
            />
            <span className="text-sm text-gray-600">
              Payment Status:{" "}
              <span className="font-medium">{pkg.paymentStatus}</span>
            </span>
          </div>
          <button
            className={`px-4 py-2 rounded-lg font-medium text-white ${packageColor.bg} hover:opacity-90 transition-opacity`}
          >
            Book Session
          </button>
        </div>
