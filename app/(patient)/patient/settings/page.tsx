...
export default function PatientSettingsPage() {
  ...
  const [medicalInfo, setMedicalInfo] = useState<MedicalInfo>({
    ...
    currentMedications: [
      ...
      { name: 'Lisinopril', dose: '10mg', schedule: 'Once daily' },
      { name: 'Atorvastatin', dose: '20mg', schedule: 'Once daily' },
    ],
    specialNotes: [
      'History of allergic reactions to antibiotics',
      'Recommended exercise: 30 mins walking daily',
  ...
  const [lifestylePrefs, setLifestylePrefs] = useState<LifestylePreferences>({
    ...
    exercise: 'Light exercise 3 times a week',
    smoking: 'Non-smoker',
  ...
  const handleEmergencyContactUpdate = (field: keyof EmergencyContact, value: string) => {
  ...
  const handleLifestyleUpdate = (field: keyof LifestylePreferences, value: string) => {
  ...
  const saveEmergencyContact = () => {
    setNotification({ type: 'success', message: 'Emergency contact updated!' });
    setIsEditing(prev => ({ ...prev, emergency: false }));
    setTimeout(() => setNotification(null), 3000);
  };

  const saveMedicalInfo = () => {
    setNotification({ type: 'success', message: 'Medical information updated!' });
    setIsEditing(prev => ({ ...prev, medical: false }));
    setTimeout(() => setNotification(null), 3000);
  };

  const saveLifestylePrefs = () => {
    setNotification({ type: 'success', message: 'Lifestyle preferences updated!' });
  ...
  const addMedication = () => {
    ...
    if (name && dose && schedule) {
      setMedicalInfo(prev => ({
        ...prev,
        currentMedications: [...prev.currentMedications, { name, dose, schedule }]
      }));
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="ml-64 p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage your profile, medical information, and preferences</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="flex-grow space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      {isEditing.profile ? (
                        ...
                        <p className="text-gray-900">{patientProfile.dateOfBirth}</p>
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                      ...
                      {isEditing.profile ? (
                        ...
                        <p className="text-gray-900">{patientProfile.phoneNumber}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing.profile ? (
                      ...
                      <p className="text-gray-900">{patientProfile.address}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing.profile ? (
                      <input
                        ...
                        onChange={(e) => handleProfileUpdate('nicPassport', e.target.value)}
                        className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    ) : (
                      <p className="text-gray-900">{patientProfile.nicPassport}</p>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-800">Medical Summary & Health Preferences</h2>
                <button
                  onClick={() => setIsEditing(prev => ({ ...prev, medical: !prev.medical }))}
                  className="flex items-center space-x-2 text-blue-600 hover:text-blue-700"
                >
                  {isEditing.medical ? (
                    <>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {isEditing.medical ? (
                    <select
                      ...
                      onChange={(e) => handleMedicalInfoUpdate('bloodGroup', e.target.value)}
                    ...
                    <p className="text-gray-900 font-medium">{medicalInfo.bloodGroup}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {medicalInfo.allergies.map((allergy, index) => (
                      <div key={index} className="flex items-center bg-red-50 text-red-700 px-3 py-1 rounded-full">
                        <span>{allergy}</span>
                        {isEditing.medical && (
                          <button
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {medicalInfo.specialNotes.length === 0 && (
                      <p className="text-gray-500 italic">No special notes recorded</p>
                    )}
                  </div>
                </div>
                ...
                {isEditing.medical && (
                  <div className="flex justify-end space-x-3 pt-4">
                    <button
                      onClick={() => setIsEditing(prev => ({ ...prev, medical: false }))}
                      className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50"
                    >
                      Cancel
                    </button>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  onClick={() => setIsEditing(prev => ({ ...prev, emergency: !prev.emergency }))}
                  className="flex items-center space-x-2 text-blue-600 hover:text-blue-700"
                >
                  {isEditing.emergency ? (
                    <>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {isEditing.emergency ? (
                    <input
                      ...
                      onChange={(e) => handleEmergencyContactUpdate('contactNumber', e.target.value)}
                      className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                  ) : (
                    <p className="text-gray-900">{emergencyContact.contactNumber}</p>
                  )}
                </div>
                ...
                {isEditing.emergency && (
                  <div className="flex justify-end space-x-3 pt-4">
                    <button
                      onClick={() => setIsEditing(prev => ({ ...prev, emergency: false }))}
                      className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50"
                    >
                      Cancel
                    </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-800">Lifestyle Preferences</h2>
                <button
                  onClick={() => setIsEditing(prev => ({ ...prev, lifestyle: !prev.lifestyle }))}
                  className="flex items-center space-x-2 text-blue-600 hover:text-blue-700"
                >
                  {isEditing.lifestyle ? (
                    <>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing.lifestyle ? (
                    <textarea
                      ...
                      onChange={(e) => handleLifestyleUpdate('diet', e.target.value)}
                      rows={2}
                      className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="e.g., Vegetarian, Low sugar, Gluten-free"
                    />
                  ) : (
                    <p className="text-gray-900">{lifestylePrefs.diet}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing.lifestyle ? (
                    <textarea
                      ...
                      onChange={(e) => handleLifestyleUpdate('exercise', e.target.value)}
                      rows={2}
                      className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="e.g., Light exercise 3 times a week"
                    />
                  ) : (
                    <p className="text-gray-900">{lifestylePrefs.exercise}</p>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    {isEditing.lifestyle ? (
                      <select
                        ...
                        onChange={(e) => handleLifestyleUpdate('smoking', e.target.value)}
                        className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                      ...
                      <p className="text-gray-900">{lifestylePrefs.smoking}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    {isEditing.lifestyle ? (
                      <select
                        ...
                        onChange={(e) => handleLifestyleUpdate('alcohol', e.target.value)}
                        className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      >
                      ...
                      <p className="text-gray-900">{lifestylePrefs.alcohol}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {isEditing.lifestyle ? (
                    <select
                      ...
                      onChange={(e) => handleLifestyleUpdate('consultationMethod', e.target.value as any)}
                      className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                    ...
                    <p className="text-gray-900">
                      {consultationMethods.find(m => m.value === lifestylePrefs.consultationMethod)?.label}
                    </p>
                  )}
                </div>
                ...
                {isEditing.lifestyle && (
                  <div className="flex justify-end space-x-3 pt-4">
                    <button
                      onClick={() => setIsEditing(prev => ({ ...prev, lifestyle: false }))}
                      className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50"
                    >
                      Cancel
                    </button>
        ...
        <div className="mt-8 flex justify-end">
          <button
            onClick={() => {
              // Export all data
              const data = { patientProfile, emergencyContact, medicalInfo, lifestylePrefs };
